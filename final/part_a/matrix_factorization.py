...
def update_u_z(train_data, lr, u, z):
    ...
    inner = c - np.transpose(u[n]) @ z[q]
    u_update = lr * inner * z[q]
    u[n] = u[n] + u_update

    z_update = lr * inner * u[n]
    z[q] = z[q] + z_update
    #####################################################################
    #                       END OF YOUR CODE                            #
    #####################################################################
...
def main():
    ...
    #####################################################################
    k_values = [1, 5, 50, 75, 100]
    val_acc = []
    for k in k_values:
        reconstructed = svd_reconstruct(train_matrix, k)
        acc = sparse_matrix_evaluate(val_data, reconstructed)
        print('k={} has a validation accuracy of {}'.format(k, acc))
        val_acc.append(acc)

    best_index = np.argmax(val_acc)
    best_k = k_values[best_index]
    best_reconstructed = svd_reconstruct(train_matrix, k)
    test_acc = sparse_matrix_evaluate(test_data, best_reconstructed)
    print('k={} has a validation accuracy of {} and test accuracy of {}'.format(best_k, val_acc[best_index], test_acc))
    #####################################################################
    #                       END OF YOUR CODE                            #
    #####################################################################
